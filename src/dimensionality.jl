...
function change_dimension(::Type{PeriodicGraph{D}}, graph::PeriodicGraph{N}, dims=_dimensionality(graph)) where {D,N}
    ...
    # nullofs = zero(SVector{N,Int})
    # nonzeroedges = [e.dst.ofs for e in edges(graph) if e.dst.ofs > nullofs]
    # if isempty(nonzeroedges) # graph is actually 0-dimensional
    #     return PeriodicGraph{D}(nv(graph), [PeriodicEdge{D}(e.src, e.dst.v, zero(SVector{D,Int})) for e in edges(graph)])
    # end
    # bigmat::Matrix{Int} = reduce(hcat, nonzeroedges; init=zeros(Int, N, 1))
    # d = rank(bigmat)
    # # d is the dimensionality of the graph, if it has a single connected component.
    # # Otherwise, we fail if d > D > actual dimensionality, although this could be fixed.
    # if d > D
    #     throw(DimensionMismatch("Graph of apparent dimensionality $d cannot be reduced to $D dimensions. Are there multiple connected components?"))
    # end
    # tmpmat = zeros(Int, N, d) # tmpmat will contain d independent vectors from nonzeroedges
    # tmpmat[:,1] .= popfirst!(nonzeroedges)
    # r = 1
    # m = length(nonzeroedges)
    # for i in 1:m
    #     r == d && break
    #     tmpmat[:,r+1] .= nonzeroedges[i]
    #     r = rank(tmpmat)
    # end
    # @assert r == d
    # mat = zeros(Int, N, N) # mat contains tmpmat as well as other vectors to make it invertible
    # mat[:,1:d] .= tmpmat
    # for i in 1:N
    #     mat[r+1,i] = 1
    #     _r = rank(mat)
    #     if _r == r
    #         mat[r+1,i] = 0
    #     else # this can only mean _r == r + 1
    #         r = _r
    #         r == N && break
    #     end
    # end
    ...
    # for (i, e) in enumerate(edges(graph))
    #     if iszero(e.dst.ofs)
    #         newedges[i] = PeriodicEdge{D}(e.src, e.dst.v, zero(SVector{D,Int}))
    #     else
    #         offset = SVector{N,Int}(invmat * e.dst.ofs)
    #         @assert iszero(@view offset[D+1:N])
    #         newedges[i] = PeriodicEdge{D}(e.src, e.dst.v, SVector{D,Int}(offset[1:D]))
    #     end
    # end
    # return PeriodicGraph{D}(nv(graph), newedges)
end
